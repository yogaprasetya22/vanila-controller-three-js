...
export class CartoonTornadoNativeVFX {
    ...
    // ponytail: scratch objects — avoids 4 allocations/frame in update() even when idle.
    // Ceiling: if multiple simultaneous tornadoes need independent scratch state, clone per-FX.
    private _dummy = new THREE.Object3D();
    private _cfgColor = new THREE.Color();
    private _baseColor = new THREE.Color();
    private _highlightColor = new THREE.Color();
    private _pColor = new THREE.Color();
    ...
    constructor(scene: THREE.Scene, camera: THREE.Camera) {
        ...
        this.outerGeometry1 = new THREE.CylinderGeometry(
            2.3,
            0.45,
            4.5,
            32,
        ...
        this.outerGeometry2 = new THREE.CylinderGeometry(
            1.9,
            0.35,
            4.5,
            32,
        ...
        this.flameGeometry = new THREE.CylinderGeometry(
            1.4,
            0.25,
            4.5,
            32,
    ...
    public spawn(x: number, y: number, z: number) {
        ...
        const createWindShader = (
            twistSpeed: number,
            twistTension: number,
            stepThreshold: number,
            colorShift: number,
            textureFlowSpeed: number,
            colorVal: THREE.Color,
            map0Tex: THREE.Texture,
        ) => {
        ...
        const cfgColor = new THREE.Color(
            CHARACTER_CONFIG.skills.tornado.hudColor,
        );

        // Layer 1: Outer wind mesh - Highlight tinted
        const outerMat1 = createWindShader(
            3.5,
            1.6,
            0.78,
            1.0,
            2.0,
            cfgColor.clone().lerp(new THREE.Color(1, 1, 1), 0.5),
            this.tex0,
        );
        // Layer 2: Inner wind mesh - Base color
        const outerMat2 = createWindShader(
            2.5,
            1.1,
            0.71,
            0.85,
        ...
        const flameMat = createWindShader(
            4.5,
            1.4,
            0.58,
            1.0,
      ...
      `,
        ...
        smokeMesh.geometry.setAttribute(
            "aColorAlpha",
            new THREE.InstancedBufferAttribute(colorAlphaArray, 4),
        );
    ...
    public update(delta: number) {
        if (this.activeFX.length === 0) return; // ponytail: early-out when idle — zero cost
        const dummy = this._dummy;
        this._cfgColor.set(CHARACTER_CONFIG.skills.tornado.hudColor);
        const baseColor = this._baseColor.copy(this._cfgColor);
        const highlightColor = this._highlightColor.set(1.0, 1.0, 1.0).lerp(
            this._cfgColor,
            0.3,
        );
        ...
        for (let i = this.activeFX.length - 1; i >= 0; i--) {
            ...
            const attr = fx.smokeMesh.geometry.getAttribute(
                "aColorAlpha",
            ...
            for (let p = 0; p < fx.particles.length; p++) {
                ...
                const pColor = this._pColor.copy(baseColor).lerp(highlightColor, lifeRatio);
