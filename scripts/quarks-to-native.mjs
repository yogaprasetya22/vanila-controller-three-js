...
function blendingName(blending) {
    const map = {
        ...
        1: "THREE.AdditiveBlending",
        2: "THREE.SubtractiveBlending",
    };
...
function fmtAlphaKeys(keys) {
    return (
        ...
        keys
            .map(
                (k) =>
                    `{ value: ${k.value.toFixed(5)}, pos: ${k.pos.toFixed(5)} }`,
            )
            .join(", ") +
        "]"
    );
...
function fmtPiecewiseBezierArray(v) {
    ...
    if (v.type === "PiecewiseBezier") {
        ...
        for (let i = 0; i < v.functions.length; i++) {
            ...
            const start = f.start;
            const end =
                i + 1 < v.functions.length ? v.functions[i + 1].start : 1.0;
            const fn = f.function;
            list.push(
                `{start: ${start}, end: ${end}, p0: ${fn.p0}, p1: ${fn.p1}, p2: ${fn.p2}, p3: ${fn.p3}}`,
            );
        }
        return `[${list.join(", ")}]`;
    }
...
if (!inputPath) {
    console.error(
        "Usage: node scripts/quarks-to-native.mjs <path-to-quarks.json>",
    );
    process.exit(1);
}
...
function collectEmitters(obj) {
    if (obj.type === "ParticleEmitter") {
        ...
        const maxLifeVal = getIntervalMax(
            ps.startLife || { type: "ConstantValue", value: 1 },
        );
        ...
        let poolCount = Math.max(
            1,
            Math.ceil(rateVal * maxLifeVal * 2.0) + burstVal + 10,
        );
        if (onlyUsedByOther) {
            poolCount = 300;
        }
        ...
        if (behaviors.EmitSubParticleSystem) {
            subEmitterTarget =
                behaviors.EmitSubParticleSystem.subParticleSystem;
        ...
        emitters.push({
            ...
            startLife: fmtInterval(
                ps.startLife || { type: "ConstantValue", value: 1 },
            ),
            startSpeed: fmtInterval(
                ps.startSpeed || { type: "ConstantValue", value: 1 },
            ),
            startSize: fmtInterval(
                ps.startSize || { type: "ConstantValue", value: 1 },
            ),
            startRotation: fmtInterval(
                ps.startRotation || { type: "ConstantValue", value: 0 },
            ),
            ...
            // FrameOverLife
            frameBezier: behaviors.FrameOverLife
                ? fmtPiecewiseBezierArray(behaviors.FrameOverLife.frame)
                : null,
            frameInterval: behaviors.FrameOverLife
                ? fmtInterval(behaviors.FrameOverLife.frame)
                : null,
            totalFrames: (ps.uTileCount || 1) * (ps.vTileCount || 1),
            // RotationOverLife
            rotSpeedBezier: behaviors.RotationOverLife
                ? fmtPiecewiseBezierArray(
                      behaviors.RotationOverLife.angularVelocity,
                  )
                : null,
            // Stretched billboard
...
for (const e of emitters) {
    if (e.subEmitterTarget) {
        ...
        const targetEmitter = emitters.find(
            (x) => x.name.replace(/[^a-zA-Z0-9]/g, "_") === targetName,
        );
        if (targetEmitter) {
...
const usedTexIndices = [
    ...new Set(emitters.map((e) => e.texIdx).filter((i) => i >= 0)),
].sort();
...
import {
  ...
  type ColorKey,
  type AlphaKey,
} from '../../graphics/effects/FXQuarksHelpers';
...
for (const i of usedTexIndices) {
    const fname =
        imgByUUID[Object.keys(imgByUUID).find((k) => imgByUUID[k].index === i)]
            ?.fname ?? `tex_${i}.png`;
...
console.log(`\nUsage in Three.js:`);
console.log(
    `  import { ${className}NativeVFX } from './vfx/${slug}/Native.ts';`,
);
